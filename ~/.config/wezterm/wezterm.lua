...
table.insert(config.keys, { key = "2", mods = "CMD", action = wezterm.action.SendKey({ key = "2", mods = "ALT" }) })
...
table.insert(
	config.keys,
	{ key = "w", mods = "CMD|SHIFT", action = wezterm.action.SendKey({ key = "w", mods = "ALT|SHIFT" }) }
)
