-- keymaps.lua (cleaned · minimal fixes only)
-- =========================================================
-- Aliases / Requires
-- =========================================================
local map = vim.keymap.set

-- 안전한 require (미설치/지연 로드 대비) — 먼저 정의
local function safe_require(mod)
  local ok, m = pcall(require, mod)
  return ok and m or nil
end

-- motions 모듈: 없으면 폴백으로 기본 동작만
local motions = safe_require("motions") or {
  smart_w_normal   = function() vim.cmd("normal! w") end,
  smart_w_visual   = function() vim.cmd("normal! w") end,
  smart_w_operator = function() return "w" end,
  smart_b_normal   = function() vim.cmd("normal! b") end,
  smart_b_visual   = function() vim.cmd("normal! b") end,
  smart_b_operator = function() return "b" end,
  lsp_supports     = function() return false end,
  project_root     = function() return vim.loop.cwd() end,
}
...
map("n", "<M-1>", function()
  ...
  if vim.fn.exists(":Neotree") == 2 then
    ...
    local neo = safe_require("neo-tree.command")
    if neo and neo.execute then
      neo.execute({ action = "toggle", position = "left", reveal = true, reveal_force_cwd = true })
    else
      vim.notify("neo-tree not available; opening netrw", vim.log.levels.WARN)
      vim.cmd("Lexplore")
    end
  end
...
map("n", "<M-W>", function()
  if vim.fn.exists(":BufferLineCloseLeft") == 2 and vim.fn.exists(":BufferLineCloseRight") == 2 then
    vim.cmd("BufferLineCloseLeft")
    vim.cmd("BufferLineCloseRight")
  else
    -- bufferline 없을 때 우회: 현재만 남기고 닫기
